...
def hasRoots(f, x0, x1, tol,sec = False):
    ...
    - Vérifie si les bornes sont bien de signe opposé (sauf sécante, sec = True)
    ...
    return [None, 0]  # Pas d'erreur détectée
...
def bissection(f, x0, x1, tol=0.5e-7, max_iter=50):
    ...
    if retour[1] != 0 or retour[0] != None:
        return retour  # Renvoie l'erreur ou la racine trouvée aux bornes 
    ...
    for _ in range(max_iter):
        x2 = (x0 + x1) / 2  # Calcul du point médian
        fx2 = f(x2)

        if abs(fx2) < tol:  # Arrêt de la fonction si précision atteinte
            return [x2, 0]
        
        if fx0 * fx2 < 0:
            x1 = x2
        else:
            x0, fx0 = x2, fx2  # Mise à jour des valeurs

    return [x2, -1]  #  pas de convergence après max_iter
...
def secante(f, x0, x1, tol=0.5e-7, max_iter=65):
    ...
    for i in range(max_iter):
        if abs(fx1 - fx0) < 1e-12:  # Évite la division par zéro 1e-12 = "zéro machine"
            return [None, -1]  # division par zéro
